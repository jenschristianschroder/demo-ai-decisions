...
const DoeExperimentScreen: React.FC = () => {
  ...
  const handleRun = async () => {
    ...
    addAudit(`Pipeline run (${result.aiBacked ? 'Azure AI Foundry' : 'mock'}) — ${result.sections.length} sections drafted from analysis.`);
      ...
      <main className="doe-exp-main">
        ...
        <section className="doe-card">
          <div className="doe-card-head">
            ...
            {report && (
              <span
                className="doe-badge doe-badge-ready"
                title={report.aiBacked
                  ? 'Narrative generated by Azure AI Foundry; statistics computed in code.'
                  : 'Deterministic mock pipeline (no Azure backend); statistics computed in code.'}
              >
                {report.aiBacked ? 'Azure AI Foundry' : 'Mock pipeline'}
              </span>
            )}
            {hasData && (
